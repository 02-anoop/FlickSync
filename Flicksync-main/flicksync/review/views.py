    ...
    if rec_service.movies is not None:
        for title in spotlight_titles:
            ...
            if not m.empty:
                ...
                if poster_path:
                    poster_url = f"/review/poster-proxy/?path={poster_path}&title={movie_row.title}&movie_id={movie_row.movie_id}&v=2"
                else:
                    poster_url = f"/review/poster-proxy/?title={movie_row.title}&movie_id={movie_row.movie_id}&v=2"
...
def recommend_view(request):
    ...
    if query:
        ...
        for r in recs:
            ...
            if poster_path:
                proxy_url = f"/review/poster-proxy/?path={poster_path}&title={r['title']}&movie_id={r['movie_id']}&v=2"
            else:
                proxy_url = f"/review/poster-proxy/?title={r['title']}&movie_id={r['movie_id']}&v=2"
        ...
        if rec_service.movies is not None:
            for title in starter_titles:
                ...
                if not m.empty:
                    ...
                    p_url = f"/review/poster-proxy/?path={p_path}&title={row.title}&movie_id={row.movie_id}&v=2" if p_path else f"/review/poster-proxy/?title={row.title}&movie_id={row.movie_id}&v=2"
